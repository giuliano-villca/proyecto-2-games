// En obj_player y obj_enemy, define la función panel_grab para robar una columna entera de paneles
function area_steal(entity_type) {
    ...
    if (found_col != -1) {
        for (var i = 0; i < array_length(global.panels); i++) {
            ...
            if (global.panels[i][found_col].propietario == target_propietario && !entity_instance) {
