...
if (colision_bloqueo != noone && colision_bloqueo.entity_type != entity_type) {
    // Destruir el aro eléctrico al impactar con el escudo
    instance_destroy();
} 
...
if (colision_invocacion != noone && variable_instance_exists(colision_invocacion, "estructura")) {
    if (colision_invocacion.entity_type != entity_type) {
        ...
        instance_destroy(); // Destruir el aro eléctrico
    }
}

// Verificar si el proyectil está en la misma fila que el objetivo
else if (colision_objetivo != noone && colision_objetivo.y == y) {
    ...
    if (!colision_objetivo.inmunidad) {
        ...
        if (damage_type == "electrico") {
            ...
            colision_objetivo.estado_duracion = 2 * room_speed;  // Parálisis por 2 segundos
        }
    }
}

// Destruir el aro eléctrico si sale de la pantalla
if (x > room_width || x < 0) {
    instance_destroy();
}
