...
if (colision_invocacion != noone && variable_instance_exists(colision_invocacion, "estructura")) {
    if (colision_invocacion.entity_type != entity_type) {
        ...
        var invocacion_base_y = colision_invocacion.y + (colision_invocacion.sprite_height / 2);
        var invocacion_row = floor((invocacion_base_y - global.grid_offset_y) / global.panel_height);
        var invocacion_col = floor((colision_invocacion.x - global.grid_offset_x) / global.panel_width);

        // Obtener el tipo de panel en el que está la base de la estructura
