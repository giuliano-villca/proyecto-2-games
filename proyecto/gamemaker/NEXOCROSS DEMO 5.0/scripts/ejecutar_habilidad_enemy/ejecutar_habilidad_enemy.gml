    ...
    if (tipo_entidad == "obj_enemy") {
        ...
        switch (habilidad) {
		        ...
		        case "reparacion":
                    ...
                    global.habilidad_estado_enemy[? habilidad] = 10 * room_speed; // 18 segundos de cooldown
                    break;
			    ...
			    case "muro":
                    muro(entity_type);
                    global.habilidad_estado_enemy[? habilidad] = 15 * room_speed; // 18 segundos de cooldown
                    break;
			    case "tornado":
                    tornado(entity_type);
                    global.habilidad_estado_enemy[? habilidad] = 2 * room_speed; // 18 segundos de cooldown
                    break;
	   	        case "aro electrico":
                    aro_electrico(entity_type);
                    global.habilidad_estado_enemy[? habilidad] = 2 * room_speed; // 18 segundos de cooldown
                    break;
	   	        case "espada infernal":
                    espada_infernal(entity_type);
                    global.habilidad_estado_enemy[? habilidad] = 2 * room_speed; // 18 segundos de cooldown
                    break;
            default:
                break;
