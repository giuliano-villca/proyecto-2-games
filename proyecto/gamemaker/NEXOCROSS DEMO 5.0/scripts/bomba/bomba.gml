...
var bomb_x, bomb_direction;
    if (entity_type == "obj_player") {
        bomb_x = x - 52	 + global.panel_width; // Posición inicial para el jugador
        bomb_direction = 0; // Dirección a la derecha
    } else if (entity_type == "obj_enemy") {
        bomb_x = x + 160 - global.panel_width; // Posición inicial para el enemigo
    ...
    // Establecer la dirección de la bomba
    bomb.direction = bomb_direction;
    bomb.attack_type = entity_type; // Guardar el tipo de entidad que lanza la bomba
}