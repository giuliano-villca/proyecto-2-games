...
if (global.panels[current_row][current_col].tipo == 4) {
    if (!inmunidad) {
        ...
        vida -= 10; // Puedes ajustar la cantidad de daño según sea necesario
		 ...
		 // Cambiar el sprite al de daño
        recibiendo_dano = true;
        dano_duracion = 0.5 * room_speed; // Duración del daño (ajustable)
...
// Estados del jugador
if (estado_actual == estado_paralizado) {
    sprite_index = spr_enemy_paralisis; // Usar sprite de parálisis
    if (estado_duracion > 0) {
        estado_duracion--;
    } else {
    ...
    sprite_index = spr_enemyv2_dano; // Cambiar a sprite de daño normal
    if (dano_duracion > 0) {
        dano_duracion--;
    } else {
    ...
    sprite_index = spr_enemyv2; // Restaurar sprite normal
}
...
if (vida <= 0) {
    ...
    global.tecla_activadora = ord("R"); // Tecla para continuar al ganar el jugador
