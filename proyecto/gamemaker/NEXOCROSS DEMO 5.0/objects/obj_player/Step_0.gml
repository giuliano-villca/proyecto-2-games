...
if (estado_actual != estado_paralizado) {
    sprite_index = spr_player_futuristav2;
...
// Lógica cuando el jugador está paralizado
if (estado_actual == estado_paralizado) {
    // Asegurarse de que el jugador no se mueva durante la parálisis
    new_x = x;
    new_y = y;

    // Reducir la duración del estado de parálisis
...
//paralisis
if (global.juego_activo && estado_actual != estado_paralizado) {
...
// Estados del jugador
if (estado_actual == estado_paralizado) {
    sprite_index = spr_player_futurista_paralizado; // Usar sprite de parálisis
    if (estado_duracion > 0) {
        estado_duracion--;
    } else {
        estado_actual = estado_normal; // Volver al estado normal
    }
} 
else if (recibiendo_dano) {
    sprite_index = spr_player_dano; // Cambiar a sprite de daño normal
    if (dano_duracion > 0) {
        dano_duracion--;
    } else {
        recibiendo_dano = false; // Salir del estado de daño
    }
} 
else {
    sprite_index = spr_player_futuristav2; // Restaurar sprite normal
}
...
// En obj_player, evento Step
    if (vida <= 0) {
    // Marcar victoria del jugador
    global.victoria_enemigo = true;
    global.juego_activo = false; // Desactiva el juego
    global.tecla_activadora = ord("I"); // Tecla para continuar al ganar el jugador

    // Destruir la instancia del enemigo
    instance_destroy();
}
