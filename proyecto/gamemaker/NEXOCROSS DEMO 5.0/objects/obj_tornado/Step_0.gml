        ...
        if (check_panel.estructura) {
            ...
            if (estructura_inst != noone && variable_instance_exists(estructura_inst, "estructura")) {
                if (estructura_inst.entity_type != attack_type) {
                    ...
                    estructura_inst.damage_received = tornado_damage;
...
if (colision_objetivo != noone) {
    ...
    if (!colision_objetivo.inmunidad) {
        ...
        fila_dano_melee(self, colision_objetivo, tornado_damage, 0.50 * room_speed, 1, 0);
        damage_applied = true; // Marcar que el daño fue aplicado
    } 
}


// Controlar la duración del tornado (2 segundos)
animation_timer += 1;
if (animation_timer >= animation_duration) {
    instance_destroy(); // Destruir el tornado después de 2 segundos
}